...
# Normalize the sentiment score to range [0, 1] for compatibility with MultinomialNB
scaler = MinMaxScaler()
final_data['sentiment_score_normalized'] = scaler.fit_transform(final_data[['sentiment_score']])

# x is independent var and y is dependent
x = final_data[['sentiment_score_normalized']]
y = final_data['class']
...
def test_all_manual_testing(data):
    ...
    for index, row in data.iterrows():
        ...
        new_def_test['sentiment_score_normalized'] = scaler.transform(new_def_test[['sentiment_score']])
        new_x_test = new_def_test[['sentiment_score_normalized']]
...
results_df.to_csv('manual_testing_results_Sentiment.csv', index=False)
